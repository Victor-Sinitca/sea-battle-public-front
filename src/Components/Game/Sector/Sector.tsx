...
export const Sector: FC<PropsType> =/* React.memo(*/ ({sector, returnMouseDown, returnMouseUp, returnMouseOver}) => {
    ...
    if (sector.sectorState.isSelected) {
        boxShadow = "inset 2px 2px 5px #374884, inset -2px -2px 5px #374884"
    }
    ...
    if (sector.sectorState.animateMove) {
        if (sector.sectorState.animateMove.shift && animation === "") {
            console.log(`animateMove: i=${sector.sectorState.animateMove.i}, j= ${sector.sectorState.animateMove.j}`)

            let styleSheet = document.styleSheets[0];
            let animationName = `keyframe${Math.round(Math.random() * 10000000)}`;
            let keyframes =
            ...
            let styleSheet = document.styleSheets[0];
            let animationName = `keyframe${Math.round(Math.random() * 1000)}`;
            let keyframes =
                `@-webkit-keyframes ${animationName} {
    ...
    return <div onAnimationEnd={handleAnimationEnd} ref={refDiv}
                style={{
                    display: "grid",
                    ...
                    animationDirection:sector.sectorState.animateMove?.isMove? "normal" :  "reverse",
                }}
        ...
        <img className={s.img} draggable={"false"} src={bonusImgMass[sector.date.bonusSector - 1]}/>}
        <div>
